...
public class UIEditbox : UIElement {
  ...
  public UIEditbox(Vector2 position, Vector2 size) : base(position) {
    this.size = size;
    float height = ResourceManager.Font.MeasureString("A").Y;
    labelPosition = new Vector2(position.X + 4, position.Y + size.Y / 2 - height / 2 + 3);
    Input.OnMouseMoved += CheckForHover;
    Input.OnLeftMouseClicked += CheckForClick;
    Input.OnKeyPressed += HandleKeyPress;
  }
  ...
  private void HandleKeyPress(Keys key) {
    if (focused && Active) {
      if (((int)key >= 48 && (int)key <= 90) || key == Keys.OemPeriod) {
        string keyPressed = key.ToString();
        if (keyPressed.StartsWith('D') && keyPressed.Length > 1) { // numberic values
  ...
  public override void Render() {
    ...
    Color foregroundColor = !hovered && !focused ? foreground : foregroundHover;
